...
## MIN Method ###
plt.figure(figsize=(10,7))
plt.title("Player PCA Dendogram (MIN method)")
dend = shc.dendrogram(shc.linkage(reduced, method = 'single'),labels= label_names)
plt.show()

agglo_cluster_min = AgglomerativeClustering(n_clusters=3, affinity='euclidean', linkage= 'single')

plt.figure(figsize=(10,7))
plt.scatter(reduced['Principal Component 1'], reduced['Principal Component 2'], c=agglo_cluster_min.fit_predict(reduced), cmap='rainbow')
...
plt.title('Agglomerative Hierarchical Clustering (MIN Method)', fontsize=25)
plt.xlabel('Principal Component 1', fontsize =20)
plt.ylabel('Principal Component 2', fontsize =20)
plt.show()