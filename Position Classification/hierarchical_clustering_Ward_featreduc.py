...
## Ward Method ###
plt.figure(figsize=(10,7))
plt.title("Player PCA Dendogram (Ward method)")
dend = shc.dendrogram(shc.linkage(reduced, method = 'ward'),labels= label_names)
plt.show()

agglo_cluster_ward = AgglomerativeClustering(n_clusters=3, affinity='euclidean', linkage= 'ward')

plt.figure(figsize=(10,7))
plt.scatter(reduced['Principal Component 1'], reduced['Principal Component 2'], c=agglo_cluster_ward.fit_predict(reduced), cmap='rainbow')
...
plt.title('Agglomerative Hierarchical Clustering (Ward Method)', fontsize=25)
plt.xlabel('Principal Component 1', fontsize =20)
plt.ylabel('Principal Component 2', fontsize =20)
plt.show()
